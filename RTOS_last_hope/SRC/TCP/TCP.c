...
#define IRQ0INTERRUPT_PRIORITY ( 3 )
...
unsigned char ucRxBuffer[ tcpMAX_REGISTER_LEN ];
	...
	TWIMessage( ucRxBuffer, i2cNO_DATA_REQUIRED, tcpDEVICE_ADDRESS, usAddress, TWIWRITE, NULL, portMAX_DELAY );

	/* Then read back from that address. */
	TWIMessage( ( unsigned char * ) pucDestination, ulLength, tcpDEVICE_ADDRESS, i2cNO_ADDR_REQUIRED, TWIREAD, xMessageComplete, portMAX_DELAY );
...
//		VICVectCntl3 = tcpEINT0_VIC_CHANNEL | tcpEINT0_VIC_ENABLE;
	...
	TWIMessage( ucDataEnableISR, sizeof( ucDataEnableISR ), tcpDEVICE_ADDRESS, tcpISR_MASK_REG, TWIWRITE, NULL, portMAX_DELAY );
...
extern volatile long lTransactionCompleted;
	...
	TWIMessage( ucDataReset,	sizeof( ucDataReset ),	tcpDEVICE_ADDRESS, tcpCOMMAND_REG, TWIWRITE, NULL, portMAX_DELAY );

	/* Now we can configure the protocol.   Here the MAC address, gateway
	address, subnet mask and IP address are configured. */
	TWIMessage( ucDataSHAR,		sizeof( ucDataSHAR ),	tcpDEVICE_ADDRESS, tcpSOURCE_HA_REG, TWIWRITE, NULL, portMAX_DELAY );
	TWIMessage( ucDataGAR,		sizeof( ucDataGAR ),	tcpDEVICE_ADDRESS, tcpGATEWAY_ADDR_REG, TWIWRITE, NULL, portMAX_DELAY );
	TWIMessage( ucDataMSR,		sizeof( ucDataMSR ),	tcpDEVICE_ADDRESS, tcpSUBNET_MASK_REG,	TWIWRITE, NULL, portMAX_DELAY );
	TWIMessage( ucDataSIPR,		sizeof( ucDataSIPR ),	tcpDEVICE_ADDRESS, tpcSOURCE_IP_REG,	TWIWRITE, NULL, portMAX_DELAY );
	...
	TWIMessage( ucDataSetTxBufSize, sizeof( ucDataSetTxBufSize ), tcpDEVICE_ADDRESS, tcpTX_MEM_SIZE_REG, TWIWRITE, NULL, portMAX_DELAY );
	TWIMessage( ucDataSetRxBufSize, sizeof( ucDataSetRxBufSize ), tcpDEVICE_ADDRESS, tcpRX_MEM_SIZE_REG, TWIWRITE, NULL, portMAX_DELAY );

	/* Send the sys init command so the above parameters take effect. */
	TWIMessage( ucDataInit,		sizeof( ucDataInit ),	tcpDEVICE_ADDRESS, tcpCOMMAND_REG, TWIWRITE, NULL, portMAX_DELAY );
...
unsigned char ucStatus;
	...
	is initialised. */
	TWIMessage( ucDataProtocol, sizeof( ucDataProtocol),tcpDEVICE_ADDRESS, tpcSOCKET_OPT_REG, TWIWRITE, NULL, portMAX_DELAY );
	TWIMessage( ucDataPort,		sizeof( ucDataPort),	tcpDEVICE_ADDRESS, tcpSOURCE_PORT_REG, TWIWRITE, NULL, portMAX_DELAY );
	TWIMessage( ucDataSockInit, sizeof( ucDataSockInit),tcpDEVICE_ADDRESS, tcpCOMMAND_REG, TWIWRITE, xMessageComplete, portMAX_DELAY );
	...
	TWIMessage( ucDataTxReadPointer, sizeof( ucDataTxReadPointer ), tcpDEVICE_ADDRESS, tcpTX_READ_POINTER_REG, TWIWRITE, NULL, portMAX_DELAY );
	vTaskDelay( tcpSHORT_DELAY );
	TWIMessage( ucDataTxWritePointer, sizeof( ucDataTxWritePointer ), tcpDEVICE_ADDRESS, tcpTX_WRITE_POINTER_REG, TWIWRITE, NULL, portMAX_DELAY );
	vTaskDelay( tcpSHORT_DELAY );
	TWIMessage( ucDataTxAckPointer,	  sizeof( ucDataTxAckPointer ),	  tcpDEVICE_ADDRESS, tcpTX_ACK_POINTER_REG, TWIWRITE, NULL, portMAX_DELAY );
	vTaskDelay( tcpSHORT_DELAY );
...
//		clear. */
//		TWIMessage( ucDataClearInterrupt, sizeof( ucDataClearInterrupt ), tcpDEVICE_ADDRESS, tcpINTERRUPT_REG, TWIWRITE, xMessageComplete, portMAX_DELAY );
//		xSemaphoreTake( xMessageComplete, tcpLONG_DELAY );
//
//		vTaskDelay( 1 );
...
//	TWIMessage( ucDataListen, sizeof( ucDataListen ), tcpDEVICE_ADDRESS, tcpCOMMAND_REG, TWIWRITE, NULL, portMAX_DELAY );
//}
/*-----------------------------------------------------------*/
...
//			in the WIZnet. */
//			TWIMessage( ucDataClearInterrupt, sizeof( ucDataClearInterrupt ), tcpDEVICE_ADDRESS, tcpINTERRUPT_REG, TWIWRITE, NULL, portMAX_DELAY );
//
...
//				a receive command. */
//				TWIMessage( ucDataReceiveCmd, sizeof( ucDataReceiveCmd ), tcpDEVICE_ADDRESS, tcpCOMMAND_REG, TWIWRITE, NULL, portMAX_DELAY );
//			}
//
...
//					TWIMessage( ucDataDisconnect, sizeof( ucDataDisconnect ), tcpDEVICE_ADDRESS, tcpCOMMAND_REG, TWIWRITE, NULL, portMAX_DELAY );
//				}
//				else
//				{
...
//	TWIMessage( ( unsigned char * ) pucTxBuffer, lTxLen, tcpDEVICE_ADDRESS, ( unsigned short ) ulSendAddress, TWIWRITE, xMessageComplete, portMAX_DELAY );
//
...
//	TWIMessage( ucTxBuffer, sizeof( ulTxAddress ), tcpDEVICE_ADDRESS, tcpTX_WRITE_POINTER_REG, TWIWRITE, xMessageComplete, portMAX_DELAY );
//
//	if( !xSemaphoreTake( xMessageComplete, tcpLONG_DELAY ) )
//	{
//		return;
...
//
//	/* Transmit! */
//	TWIMessage( ucDataSend, sizeof( ucDataSend ), tcpDEVICE_ADDRESS, tcpCOMMAND_REG, TWIWRITE, xMessageComplete, portMAX_DELAY );
//
//	if( !xSemaphoreTake( xMessageComplete, tcpLONG_DELAY ) )
//	{
//		return;
